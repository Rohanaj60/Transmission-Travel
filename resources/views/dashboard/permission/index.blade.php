<x-app-layout>
    <div class="flex justify-between">
        ...
        <div class="p-4">
            ...
            <Link href="{{ route('permissions.create') }}"
                class="px-4 py-2 bg-orang hover:bg-indigo-700 rounded text-white">New Permission</Link>
            {{-- @endrole --}}
        </div>
    </div>
    <x-splade-table :for="$permissions">
        ...
        <div class="space-x-2">
            <Link href="{{ route('permissions.edit', $permissions) }}"
                class="text-green-400 hover:text-green-700 font-semibold">
            Edit
            </Link>
            <Link href="{{ route('permissions.destroy', $permissions) }}" method="DELETE" confirm="Delete the Product"
